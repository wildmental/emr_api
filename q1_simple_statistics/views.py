    ...
    def stat(self, request):
        ...
        per_ethnic = {ethnic_cnts["person__ethnicity_source_value"]: ethnic_cnts["ethnic_cnt"]
                      for ethnic_cnts in q.values("person__ethnicity_source_value", "person__ethnicity_concept_id")
                                          ...
                                          .order_by("-ethnic_cnt")}
        per_visit_time_age_group = {}
        visit_list = list(q.values("person__year_of_birth", "visit_start_date"))
        for visit in visit_list:
            visit_time_age_group = \
                ((int(str(visit["visit_start_date"])[:4]) - visit["person__year_of_birth"]) / 10).__floor__() * 10
            visit_time_age_group_name = f'{visit_time_age_group}y to {visit_time_age_group + 9}y'
            if visit_time_age_group_name in per_visit_time_age_group.keys():
                per_visit_time_age_group[visit_time_age_group_name] += 1
            else:
                per_visit_time_age_group[visit_time_age_group_name] = 1
        per_current_age_group = {}
        ...
        for age_group, year_interval_10 in zip(range(max_group_gap, -1, -10),
            ...
            per_current_age_group[f"{age_group}y to {age_group+9}y"] = cnt['person__year_of_birth__count']
        response_data = {"visits per type": per_type,
                         "visits per gender": per_gender,
                         "visits per race": per_race,
                         "visits per ethnic": per_ethnic,
                         "visits per age group when the visit occurred": per_visit_time_age_group,
                         "visits per current age group": per_current_age_group,
                         }
        return Response(response_data, status=status.HTTP_200_OK)
